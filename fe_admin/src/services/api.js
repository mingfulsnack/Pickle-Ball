...
api.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('adminToken'); // Use adminToken for admin
    if (token) {
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      console.log('401 Unauthorized - admin token may be expired');
...
export const authAPI = {
  login: (credentials) => api.post('/auth/admin-login', credentials), // Admin specific endpoint
